...
def build_smart_recommendations(metrics: dict[str, Any], *, timeframe: str) -> dict[str, Any]:
    ...
    if "data_quality_gate" in gates or label_q.get("fail_hard"):
        add(
            ...
            knobs={"barrier_atr_multiplier": 1.85, "dq_gate_hard": True},
        )
    if fit == "overfitting" or any("overfit" in g for g in gates):
        add(
            ...
            knobs={"lgb_max_depth": 3, "lgb_reg_lambda": 6.0, "top_features": 40},
        )
    if fit == "underfitting":
        add(
            2,
            ...
            knobs={"lgb_num_leaves": 20, "top_features": 64},
        )
    if "val_fold_liquidity" in gates or "deploy_holdout_trades" in gates:
        add(
            ...
            knobs={"confidence_quantile": 0.78, "cost_edge_multiple": 1.15},
        )
    if float(cls.get("roc_auc_ovr", 1) or 1) < 0.52:
        add(
            ...
            knobs={"use_meta_labeling": True, "model_zoo_enabled": True},
        )
    if float(fin.get("expectancy", 0) or 0) <= 0:
        add(
            ...
            knobs={"latency_bars": 1, "cost_edge_multiple": 1.4},
        )
    if regime.get("stable") is False or "regime_unstable" in gates:
        add(
            ...
            knobs={"regime_filter": True, "validation_mode": "cpcv_lite"},
        )
    if stress.get("robust") is False or "stress_fragile" in gates:
        add(
            ...
            knobs={"spread_pips": 40.0, "slippage_pips": 8.0},
        )
    if mc.get("stable") is False:
        add(
            ...
            knobs={"target_trade_rate": 0.045, "min_trades_oos": 30},
        )
    if (feat_x.get("warnings") or []):
        add(
            ...
            knobs={"stable_feature_selection": True, "shap_enabled": True},
        )
    if label_q.get("flags", {}).get("high_noise"):
        add(
            ...
            knobs={"barrier_atr_multiplier": 2.0, "use_meta_labeling": True},
        )
    if zoo.get("winner") and nested.get("best_family") and str(zoo["winner"]) != str(nested.get("best_family")):
        add(
            ...
            knobs={"nested_hp_per_fold": True, "use_ensemble": True},
        )
    ...
    return {
        ...
        "primary_code": top[0]["code"] if top else None,
    }
